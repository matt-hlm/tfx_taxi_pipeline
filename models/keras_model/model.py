...
def _get_labels(feature_dict):
    label_dict = {}
    for label_key in features.LABEL_KEY:
        label_dict[label_key] = feature_dict.pop(label_key)
...
def _input_fn(file_pattern, data_accessor, tf_transform_output, batch_size=200):
    ...
    dataset = dataset.map(_get_labels)
    return dataset 
...
def _build_keras_model(hidden_units, learning_rate):
  ...
  output_columns = features.LABEL_KEY
...
def run_fn(fn_args):
  ...
  def format_dataset_manual(dataset):
    iterator = dataset.__iter__()
    dataset_component = iterator.get_next()
    input_train_dataset = dataset_component[0] # input dataset is the first component
    #print(f"Input from TFX: {input_train_dataset}")
    output_train_dataset = dataset_component[1] # output dataset is the second component
    #print(f"Output from TFX: {output_train_dataset}")
    
    # Building out and formatting the input dataset
    input_train =[] # initialize as multi-dimension array
    for key in input_train_dataset.keys():
        data_entry = np.array(input_train_dataset[key].numpy().astype('float32'))
        data_entry = np.squeeze(data_entry) # enforce single-dimensionality of field
        input_train.append(data_entry) # add field to overall training set
    # Final conversion to Tensor
    input_train=np.transpose(np.array(input_train))
    input_train = tf.convert_to_tensor(input_train)
    print(f"Input dataset shape: {input_train.shape}")
    print(input_train)

    
    # Building out and formatting the input dataset
    output_train =[] # initialize as multi-dimension array
    for key in output_train_dataset.keys():
        data_entry = np.array(output_train_dataset[key].numpy().astype('float32'))
        data_entry = np.squeeze(data_entry) # enforce single-dimensionality of field
        output_train.append(data_entry) # add field to overall training set
    # Final conversion to Tensor
    output_train=np.transpose(np.array(output_train))
    output_train = tf.convert_to_tensor(output_train)
    print(f"Output dataset shape: {output_train.shape}")
    print(output_train)
    
    # Return both formatted datasets
  ...
  train_input, train_output = format_dataset_manual(train_dataset)
  
  eval_dataset = _input_fn(fn_args.eval_files, fn_args.data_accessor,
                          tf_transform_output, constants.EVAL_BATCH_SIZE)
  
  eval_input, eval_output = format_dataset_manual(eval_dataset)
